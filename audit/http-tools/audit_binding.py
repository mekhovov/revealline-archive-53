"""Read-only, fail-closed intake of the actual Archive53 initial-deployment receipts."""
import hashlib
import io
import json
import os
...
BASE = 'https://mekhovov.github.io/revealline-archive-53/'
REPO = 'mekhovov/revealline-archive-53'
TREE = '6abe465ee6775405df2441b2cda76f8c8bfd20d5'
INVENTORY_SHA = 'af53bba40a5726c0959c1433f0e03f4813eca8f6ef4086f8d82dd2900ff326ec'
LOCK_SHA = 'dee5233fed141efe98d93ffed6c23491d3e24694232966d83c13983f551680a3'
...
def candidate_inventory():
    ...
    require(len(rows) == len(current) == 1085 and sum(r['bytes'] for r in rows) == 590842992, 'Wrong finite inventory')
...
def validate_execution_binding():
    ...
    require(request['format'] == 'revealline-archive53-http-request.v1' and request['reviewed'] is True, 'Actual request requires explicit review')
    ...
    require(artifact['name'] == 'archive53-verification-receipts' and artifact['expired'] is False and artifact['workflow_run']['id'] == run_id and artifact['workflow_run']['head_sha'] == commit, 'Wrong receipt artifact run')
    ...
    with zipfile.ZipFile(io.BytesIO(zip_raw)) as archive:
        names = ['receipt.json', 'expected-inventory.json', 'zip-receipt-v0.84.0.json']
